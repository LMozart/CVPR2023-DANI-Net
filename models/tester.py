...
class DepthBaseTester(object):
    ...
    def load_checkpoints(self, cfg):
        ...
        self.params.load_state_dict(ckpt['sigma_state_dict'])
    ...
    def eval(self, pred, data):
        ...
        nml_err_map = nml_err_map[self.bbox_int[0]:self.bbox_int[1], self.bbox_int[2]:self.bbox_int[3]]
        
        nml_stack = np.concatenate((gt_nml_map, est_nml_map, nml_err_map), axis=1)
        plots["nml"] = nml_stack
        plots["est_nml"]     = est_nml_map
        plots["est_nml_err"] = nml_err_map
